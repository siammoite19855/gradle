...
==== TestNG output may be different when using a version before 6.9.13.3
As a part of the `AbstractTestTask` refactoring, the integration with TestNG has been updated.
Gradle now relies on a correctly functioning `IClassListener` integration to report the correct hierarchy of test classes and methods.
This may lead to different output when using TestNG versions before 6.9.13.3.
