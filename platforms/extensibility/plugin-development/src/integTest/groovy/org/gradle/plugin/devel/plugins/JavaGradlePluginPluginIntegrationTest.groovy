...
class JavaGradlePluginPluginIntegrationTest extends WellBehavedPluginTest {
    ...
    def "can define feature flags on plugins"() {
        ...
        def configurationCacheValue = ""
        if (configurationCacheFlag != null) {
            configurationCacheValue = "configurationCache = $configurationCacheFlag"
        }

        def isolatedProjectsValue = ""
        if (isolatedProjectsFlag != null) {
            isolatedProjectsValue = "isolatedProjects = $isolatedProjectsFlag"
        }
            ...
            gradlePlugin {
                plugins {
                    testPlugin {
                        ...
                        implementationClass = 'com.xxx.TestPlugin'
                        supportedFeatures {
                            $configurationCacheValue
                            $isolatedProjectsValue
                        }
                    }
                }
            }
        ...
        where:
        [configurationCacheFlag, isolatedProjectsFlag] << [
            [true, false, null],
            [true, false, null]
        ].combinations()
    }
